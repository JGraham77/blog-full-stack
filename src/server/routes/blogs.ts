...
router.post('/', async (req, res) => {
    ...
    if (!title || typeof title !== 'string' || title.length > 64 || !content || typeof content !== 'string' || content.length > 65_535) {
...
router.put('/:id', async (req, res) => {
    ...
    if (!title || typeof title !== 'string' || title.length > 64 || !content || typeof content !== 'string' || content.length > 65_535) {
        res.status(400).json({ message: 'Invalid content/title or title greater than 64 characters or content greater than 65,535 characters' });
        return;
    }
